...
async function fetchIvr() {
  ...
  try {
    ...
    editCluster.value = ivr.value?.cluster ?? 'default'
    editDescription.value = ivr.value?.description ?? ''
  } catch (err) {
...
async function saveEdit(e) {
  ...
  saving.value = true
  try {
    await getApiClient().put(`ivrs/${encodeURIComponent(pkey.value)}`, {
    ...
    const msg =
      ...
      err.data?.message ??
      err.message
    saveError.value = msg || 'Failed to update IVR'
  } finally {
    saving.value = false
  }
}
...
async function doDelete() {
  if (!confirm(`Delete IVR "${pkey.value}"? This cannot be undone.`)) return
  deleteError.value = ''
  deleting.value = true
  try {
